...
On this page, I only focus on **qPCR data** because BCoV and PMMoV substantially increased when we transitioned to dPCR.
...
#save(data.H12.backup, file="data/data.H12.backup.RData")
#load("data/data.H12.backup.RData")
```
...
data$sc2_dPMMoV.sqrt2<-data$avg_sars_cov2_conc/sqrt(data$pmmov_conc/mean(data$pmmov_conc))
data$sc2_mPMMoV.sqrt2<-data$avg_sars_cov2_conc*sqrt(data$pmmov_conc/mean(data$pmmov_conc))
...
data$sc2_dBCoV.sqrt2<-data$avg_sars_cov2_conc/sqrt(data$bcov_rec_rate/mean(data$bcov_rec_rate))
data$sc2_mBCoV.sqrt2<-data$avg_sars_cov2_conc*sqrt(data$bcov_rec_rate/mean(data$bcov_rec_rate))

data$sc2_dPMMoVBCoV.sqrt2<-data$avg_sars_cov2_conc/sqrt((data$bcov_rec_rate/mean(data$bcov_rec_rate))*(data$pmmov_conc/mean(data$pmmov_conc)))
data$sc2_mPMMoVBCoV.sqrt2<-data$avg_sars_cov2_conc*sqrt((data$bcov_rec_rate/mean(data$bcov_rec_rate))*(data$pmmov_conc/mean(data$pmmov_conc)))
...
data.select.1<-data.select.1 %>% 
  select(wwtp_name, diff_pmmov_conc, diff_bcov_rec_rate, diff_average_flow_rate, diff_ph, diff_temperature, diff_BOD5, diff_TSS, difference.log)%>% 
...
data.select.1.sc<-scale(data.select.1 %>% 
  select(diff_pmmov_conc, diff_bcov_rec_rate, diff_average_flow_rate, diff_ph, diff_temperature, diff_BOD5, diff_TSS, difference.log), center = TRUE, scale = TRUE)
...
The main conclusion from the output below is that the WWTPs do not behave the same. However, overall the flow and BCoV are frequently identify to be good variables to decrease the variance in the diff(WW-case). 


The flow was identify in 4 out of the 6 WWTPs that have combined sewer systems (when it rains, the sewage flow increase). 
...
for(i in 1:nlevels(as.factor(data.select.1.sc$wwtp_name))){
  ...
  } else if(wwtp %in% c("Superior", "Watertown")){
    data.rf3=data.select.1.sc %>% filter(wwtp_name == wwtp) %>% select(-c(wwtp_name, diff_ph, diff_temperature))
...
plot_list[[i]] <- ggplot(ImpData, aes(x=Var.Names, y=`%IncMSE`)) +
  ...
  if(i==1){
    ...
    IncMSE<-rbind(ImpData, IncMSE)
  }
}
write.table(IncMSE, "IncMSE_qPCR.tsv", sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
```
