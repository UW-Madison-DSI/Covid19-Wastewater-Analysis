...
spikes in Wisconsin communities to better inform decision making policy. 


```{r,  fig.cap = "Wastewater with flagged dates and quantintle info", PlotHeight = 25}
...
```{r, fig.cap = "Case slope", PlotHeight = 17}
library(DSIWastewater)
...
between normal fluctuation and and the start of a spike.

A single case flag is suspect but 5 case flags in a row is much more likely to 
represent a true rise in Covid-19 in the population. This confidence is not currently
being represented in the system hurting effectiveness. 

The wastewater flagging does a better job overall then the case method due to
The 90 day rolling 80th percentile filter. however the CDC method is very bad
at detecting long term trends meaning that the filter only masks the underlying
issue.

\pagebreak


```{r fig.cap = "waste counts", PlotHeight = 15}
...
is strongly affected by natural variance in the system. The right plot shows that 
...
```{r fig.cap = "Zoomed in case slope plot", PlotHeight = 15}
