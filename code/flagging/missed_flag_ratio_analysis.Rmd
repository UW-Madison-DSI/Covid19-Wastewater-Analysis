...
DistSummaryMainSite <- rawDateDistDF%>%
  ...
  summarise(Mean = mean(FlagError, na.rm = TRUE),
            ...
            missed_percent = 100*mean(abs(FlagError) > edgeThresh, na.rm = TRUE),
...
DistSummaryMainSite%>%
  ...
  ggplot(aes(x = MeanErrorSquard, y = missed_percent, 
...
a <- QuantDistSummarySite%>%
  #filter(missed_percent != 0)%>%
  ggplot(aes(x = MeanErrorSquard, y = missed_percent, color = factor(pop), info = n))+
  geom_point(size = 2)+
...
a <- QuantDistSummarySite%>%
  #filter(missed_percent != 0)%>%
  ggplot(aes(x = MeanErrorSquard, y = missed_percent, color = factor(sampleRate), info = n))+
  geom_point(size = 2)+
  geom_smooth(se = FALSE, method = "lm")#+
  #scale_x_log10()
ggplotly(a)
...
rawDateDistDF%>%
  left_join(city_data)%>%
  group_by(pop, sampleRate)%>%
  summarise(n = n())%>%
  ggplot(aes(x = pop, y = sampleRate))+
