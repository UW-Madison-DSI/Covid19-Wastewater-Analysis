...
DistMean <- DateDistDF%>%
  ...
  tidyr::pivot_longer(cols = dep_flags,
                      ...
                      names_sep = "_")
library(plotly)
plot_ly(DistMean, x = ~window, y = ~quant, z = ~Mean, color = ~FlagType)%>% add_markers()
...
plot_ly(DistVar, x = ~window, y = ~quant, z = ~Var, color = ~FlagType)%>% add_markers()
```
