...
Ccf <- function(wastecasedf, startdate, enddate, lag, name){
  tempwc <- subset(wastecasedf, as.Date(date)> as.Date(startdate) & as.Date(date) < as.Date(enddate))
  tempwastedf <- subset(wastedf, as.Date(date)> as.Date(startdate) & as.Date(date) < as.Date(enddate))
...
#varsplit  = 1
  if(varsplit == 1){
    ...
    for(v in 1:3){#length(covarstarts)
      cordatasubset <- subset(cordata, as.Date(date)> as.Date(covarstarts[v]) & as.Date(date) < as.Date(covarends[v]))
      for(j in 0:9){
        for(i in 0:9){
...
#wasteinput = waste
#caseinput = cases
heatmapfunc <- function(scale, timePeriods,wasteinput,caseinput,list,lod=FALSE,week){
...
heatmapfunc("kendall_offset",0,waste,cases,"pop",TRUE,TRUE)
heatmapfunc("kendall_offset",0,waste,cases,"pop",TRUE,TRUE)

heatmapfunc("kendall_offset",0,waste %>% filter(pcr_type == "qPCR"),cases,"pop",TRUE,TRUE)
heatmapfunc("kendall_offset",0,waste %>% filter(pcr_type == "dPCR"),cases,"pop",TRUE,TRUE)
heatmapfunc("kendall_offset",0,waste %>% filter(pcr_type == "droplet digital PCR"),cases,"pop",TRUE,TRUE)
