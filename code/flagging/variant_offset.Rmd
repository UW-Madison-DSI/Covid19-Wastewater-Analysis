...
for(j in 0:14){
  for(i in 0:14){
    if(i == 0 || j == 0){#avoid subtracting when there is nothing
      wctemplm <- wastecaseccfmerg %>% mutate(tempsum = roll_sum(conf_case,i,align = "left",fill = NA) + roll_sum(conf_case,j,align = "right",fill = NA))
    } else {
    wctemplm <- wastecaseccfmerg %>% mutate(tempsum = roll_sum(conf_case,i,align = "left",fill = NA) + roll_sum(conf_case,j,align = "right",fill = NA) - roll_sum(conf_case,1,align = "center",fill = NA) ) 
    }
...
heatmapcor <- rsquareddf[-1,] %>% ggplot(aes(x = left, y = right, fill = rsquared))+geom_tile() + scale_fill_gradientn(colours=rainbow(7)) + xlab("future") + ylab("past")
ggplotly(heatmapcor)
heatmapcor
...
#wasteinput = waste
#caseinput = cases
heatmapfunc <- function(scale, timePeriods,wasteinput,caseinput, lod=FALSE){
  ...
  for(j in regionslist){
    for(i in 1:length(start)){
      ...
      wastervtemp <- wastervtemp %>% summarise(N1 = mean(N1),
              N2 = mean(N2),
              PMMoV = mean(PMMoV),
              meanLOD = rowMeans(wastervtemp[,c('n1_lod', 'n2_lod')], na.rm=TRUE) )
      
      if(lod){
        wastervtemp <- wastervtemp %>% mutate(geo_mean = exp((log(N1) + log(N2))/2)) %>% 
          drop_na(geo_mean) %>% filter(geo_mean > meanLOD / 2)
      } else {
        wastervtemp <- wastervtemp %>% mutate(geo_mean = exp((log(N1) + log(N2))/2)) %>% 
          drop_na(geo_mean)
      }
  ...
  heatmaprvt <- ggplot(data = outputrvt, aes(fill = output,)) +
    ...
    xlab(case_when(timePeriods == 0 ~ paste("Variants (Since August 1st 2020)"),
    ...
    scale_y_discrete(labels=c("0" = "Southeastern", "1" = "Northern","2" = "Northeastern","3" = "Western","4" = "Southern","5" = " ",  "6" = "All Sites","7" = " ")) +
   labs(fill="Days")
...
heatmapfunc("pearson_offset",0,waste,cases,TRUE)
ggplotly(heatmapfunc("pearson_offset",0,waste,cases))

##ggplotly(heatmapfunc("rsquared_cor",0,waste,cases))
##ggplotly(heatmapfunc("rsquared_cor",6,waste,cases))
#ggplotly(heatmapfunc("rsquared_cor",4,waste,cases))
#ggplotly(heatmapfunc("rsquared_cor",3,waste,cases))
##ggplotly(heatmapfunc("rsquared_cor",2,waste,cases))

ggplotly(heatmapfunc("pearson_offset",0,waste,cases))
#ggplotly(heatmapfunc("pearson_offset",6,waste,cases))
#ggplotly(heatmapfunc("pearson_offset",2,waste,cases)) 
ggplotly(heatmapfunc("pearson_offset",1,waste,cases)) 

#ggplotly(heatmapfunc("offset_ccf",0,waste,cases))
#ggplotly(heatmapfunc("offset_ccf",6,waste,cases))
#ggplotly(heatmapfunc("offset_ccf",2,waste,cases))#using ccf

#ggplotly(heatmapfunc("offset_rcor",2,waste,cases))#using r cor
#ggplotly(heatmapfunc("offset_rcor",0,waste,cases))

ggplotly(heatmapfunc("spearman_offset",0,waste,cases)) 
ggplotly(heatmapfunc("spearman_offset",6,waste,cases)) 
ggplotly(heatmapfunc("spearman_offset",3,waste,cases)) 
ggplotly(heatmapfunc("spearman_offset",1,waste,cases)) 
ggplotly(heatmapfunc("spearman_offset",3,waste,cases,TRUE)) 

ggplotly(heatmapfunc("kendall_offset",0,waste,cases)) 
ggplotly(heatmapfunc("kendall_offset",6,waste,cases)) 
ggplotly(heatmapfunc("kendall_offset",3,waste,cases)) 
ggplotly(heatmapfunc("kendall_offset",1,waste,cases)) 
ggplotly(heatmapfunc("kendall_offset",3,waste,cases,TRUE))
...
#Look at population grouping instead.
#Break output into multiple code blocks to make it easier to find/parse

#Fix problem with cumulative columns.	
#Drop max values or label them?

#variants with/without lod
#Fix all site
#Sort by population
...
heatmapfuncpop <- function(scale, timePeriods,wasterv,caserv){
  ...
  poplist <- unique(wasterv$popgroup)
  outputrvt <- data.frame(matrix(ncol = 1, nrow = 0))
