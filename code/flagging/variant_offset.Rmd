...
#wasteinput = waste
#caseinput = cases
heatmapfunc <- function(scale, timePeriods,wasteinput,caseinput){
  if(scale == "rsquared"){
    outputSelect = 0
    ...
    outputSelect = 3
  } else if (scale == "offsetccf"){
    outputSelect = 4
  } else if (scale == "offsetrcor"){
    outputSelect = 5
    }
  
  wasterv <- wasteinput
  caserv <- caseinput
  regionslist <- unique(wasterv$regions)
...
##ggplotly(heatmapfunc("rsquared",0,waste,cases))
##ggplotly(heatmapfunc("rsquared",6,waste,cases))
#ggplotly(heatmapfunc("rsquared",4,waste,cases))
#ggplotly(heatmapfunc("rsquared",3,waste,cases))
##ggplotly(heatmapfunc("rsquared",2,waste,cases))

#ggplotly(heatmapfunc("offset",0,waste,cases))
ggplotly(heatmapfunc("offset",6,waste,cases))#pearson cor
ggplotly(heatmapfunc("offset",2,waste,cases)) 

ggplotly(heatmapfunc("offsetccf",0,waste,cases))
ggplotly(heatmapfunc("offsetccf",6,waste,cases))
ggplotly(heatmapfunc("offsetccf",2,waste,cases))#using ccf

ggplotly(heatmapfunc("offsetrcor",2,waste,cases))#using r cor
ggplotly(heatmapfunc("offsetrcor",0,waste,cases))
#non parametric corrilation test
#spearman
```
```{r}
ggplotly(heatmapfunc("offset",1,waste,cases)) 
```
