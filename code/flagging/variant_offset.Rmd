...
covarstarts <- covarstarts[-6]
covarends <- covarends[-6]
covarnames <- covarnames[-6]
```
...
HFGwaste <- HFGwaste %>% mutate(n1_lod = case_when(HFGwaste$N1LOD == FALSE ~ HFGwaste$N1+1,
                                                   .default = HFGwaste$N1-1),
                                n2_lod = case_when(HFGwaste$N2LOD == FALSE ~ HFGwaste$N2+1,
                                                   .default = HFGwaste$N2-1))
...
```{r}
heatmapcorfunc <- function(cordata,varsplit){
#cordata <- wastecaseccfmerg
#varsplit  = 1
  if(varsplit == 1){
    rsquareddf <- data.frame(matrix(ncol = 1, nrow = 0))
    for(v in 1:3){#length(covarstarts)
      cordatasubset <- subset(cordata, anydate(date)> anydate(covarstarts[v]) & anydate(date) < anydate(covarends[v]))
      for(j in 0:9){
        for(i in 0:9){
          wctemplm <- cordatasubset %>% 
          ...
          templm <- lm(log(tempsum+1)~log(geo_mean+1), data=wctemplm) # :variant :region
          
          new <- c(i,j,summary(templm)$r.squared,paste(covarnames[v]))
...
```{r}
heatmapcorfunc(wastecaseccfmerg,0) + theme(legend.title= element_blank())
#heatmapcorvariants <- heatmapcorfunc(wastecaseccfmerg,1)
#heatmapcorvariants$rsquared <- as.numeric(heatmapcorvariants$rsquared)
#heatmapcorvariants %>% ggplot(aes(x = left, y = right, fill = rsquared)) + 
#  geom_tile(size = 0.1) +
...
```{r}
heatmapMadisonW <- waste %>% filter(grepl("Madison", site)) %>% group_by(date) %>%
  summarise(N1 = mean(N1),
              N2 = mean(N2),
              PMMoV = mean(PMMoV),
...
heatmapMadisonC <- cases %>% filter(grepl("Madison", site)) %>% group_by(date) %>%
  summarise(conf_case = mean(conf_case)) 

heatmapMadison <- merge(heatmapMadisonW,heatmapMadisonC, by = "date")
#heatmapcorfunc(heatmapMadison,0)
#heatmapcorvariants <- heatmapcorfunc(heatmapMadison,1)
#heatmapcorvariants$rsquared <- as.numeric(heatmapcorvariants$rsquared)
#heatmapcorvariants %>% ggplot(aes(x = left, y = right, fill = rsquared)) + 
#  geom_tile(size = 0.1) +
#  facet_wrap(~variant, ncol=2) +
...
#wasteinput = waste
#caseinput = cases
heatmapfunc <- function(scale, timePeriods,wasteinput,caseinput,list,lod=FALSE,testSwitch,week){
  ...
  if(testSwitch == "qPCR"){
    ...
    endday <- as.numeric(as.Date("2022-01-01"))
    wasterv <- subset(wasterv, anydate(date)> anydate(firstday) & anydate(date) < anydate(endday))
    caserv <- subset(caserv, anydate(date)> anydate(firstday) & anydate(date) < anydate(endday))
  } else if(testSwitch == "dPCR"){
    firstday <- as.numeric(as.Date("2022-01-01"))
  ...
  heatmaprvt <- ggplot(data = outputrvt, aes(fill = output,)) +
    ...
    ylab(case_when(list == "region" ~ "Region",
                   .default = "Population")) +
    ggtitle(case_when(lod == TRUE ~ paste(scale, "values above LOD", testSwitch),
          .default = paste(scale, "using all N1/N2", testSwitch) ) )  +
   labs(fill="") +#Days wastewater is offset from cases #was: Offset in days
    geom_text(aes(x = (as.Date(start + firstday) + (as.Date(end + firstday) - as.Date(start + firstday)) / 2),
                  y = .75, 
...
heatmapfunc("kendall_offset",0,waste,cases,"pop",TRUE,"Both tests",TRUE)


heatmapfunc("kendall_offset",0,waste %>% filter(pcr_type == "qPCR"),cases,"pop",TRUE,"Both tests",TRUE)
heatmapfunc("kendall_offset",0,waste %>% filter(pcr_type == "dPCR"),cases,"pop",TRUE,"Both tests",TRUE)
heatmapfunc("kendall_offset",0,waste %>% filter(pcr_type == "droplet digital PCR"),cases,"pop",TRUE,"Both tests",TRUE)


unique((waste %>% filter(pcr_type == "droplet digital PCR"))$site)

```

```{r}
#smoothed/outlier removal
...
HFGcase$ConfirmedCases[HFGcase$ConfirmedCases == -999] <- 2.5
HFGcase$ConfirmedCases[is.na(HFGcase$ConfirmedCases)] <- 0
HFGcase <- HFGcase %>% rename(conf_case = ConfirmedCases)
heatmapfunc("kendall_offset",1,HFGwaste,HFGcase,"pop",FALSE,"Both tests")


#madison only

heatmapfunc("kendall_offset",0,waste %>% filter(grepl("Madison", site)),cases %>% filter(grepl("Madison", site)),"pop",FALSE,"Both tests")
heatmapfunc("kendall_offset",0,waste %>% filter(grepl("Madison", site)),cases %>% filter(grepl("Madison", site)),"pop",FALSE,"qPCR")
heatmapfunc("kendall_offset",0,waste %>% filter(grepl("Madison", site)),cases %>% filter(grepl("Madison", site)),"pop",FALSE,"dPCR")

heatmapfunc("kendall_offset",6,waste %>% filter(grepl("Madison", site)),cases %>% filter(grepl("Madison", site)),"pop",FALSE,"Both tests")
heatmapfunc("kendall_offset",2,waste %>% filter(grepl("Madison", site)),cases %>% filter(grepl("Madison", site)),"pop",FALSE,"Both tests")
heatmapfunc("kendall_offset",1,waste %>% filter(grepl("Madison", site)),cases %>% filter(grepl("Madison", site)),"pop",FALSE,"Both tests")
```

```{r}


subset(cases, select = -c(site) ) %>%  group_by(date) %>% 
  summarise(across(everything(), list(sum))) %>%
  ggplot() +
  geom_point(aes(x=date, y=tests_1, color = "tests"))
```
