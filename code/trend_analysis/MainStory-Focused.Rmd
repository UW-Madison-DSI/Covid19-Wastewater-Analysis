...
yt <- FullDF%>%
  filter(site == "Madison")%>%
  ungroup()%>%
  mutate(N1 = log(sars_cov2_adj_load), case = log(FirstConfirmed.Per100K), true = NA)%>%
  arrange(date)%>%
  select(case, N1)%>%
  data.matrix()%>%
  t()
...
dt = c(0,0)#update equation intercept
ct = c(0,0)#messurement update intercept
Tt = matrix(c(.3, 0, 
              0, .7), nrow = 2)#update equation matrix
Zt = diag(2)#measurement equation matrix
HT = matrix(rep(10, 4), nrow = 2)
HHt = HT %*% t(HT)
GT = matrix(c(.1, 0, 
              0, .263), nrow = 2)
GGt = GT %*% t(GT)
