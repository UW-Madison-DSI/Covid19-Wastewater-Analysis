...
```{r plot 2}
Graph_DF%>%
  mutate(LOD = ifelse(LOD, "Above LOD", "Below LOD"))%>%
  ggplot(aes(x = exp(avg_sars_cov2_conc), y = exp(case_rate -.676*log(pop)) , color = LOD))+
...
df_LOD%>%
  ...
  ggplot(aes(x = exp(avg_sars_cov2_conc), y = exp(n_case_rate)))+
  geom_point(size = size, alpha = alpha)+
