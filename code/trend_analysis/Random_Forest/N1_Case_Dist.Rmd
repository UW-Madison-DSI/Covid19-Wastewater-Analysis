---
title: "Data Precision During the Pandemic: Evaluating Normalization Approaches for Reliable COVID-19 Quantification in Wastewater"
output:
...
---
Authors list:
Marlin Lee, Dagmara Antkiewicz, Adélaïde Roguet, Kayley Janssen, Martin Shafer, Abe Megahed, Kyllan Wunder


### Abstract

The COVID-19 pandemic has presented unprecedented challenges to global public
health, requiring rapid and accurate monitoring strategies. Wastewater-based epidemiology
(WBE) has emerged as a promising approach for early community detection of SARS-CoV-2,
the virus responsible for COVID-19. However, challenges remain in using the measured
wastewater concentrations to reliably predict new outbreaks or call significant community viral
load changes. The intrinsic complexity and variability of wastewater surveillance data
necessitates the use of normalization approaches including viral markers, laboratory controls,
and general collection source factors to establish and improve the connection between
wastewater concentration and community health / trends.
In this study, we investigated various characteristics of the collected samples, including
human fecal marker (PMMoV) concentration, reported measurement LOD (limit of detection),
and laboratory viral recovery control (BCoV), in an effort to reduce variance in the SARS-CoV-2
measurement and to enhance its connection to collected case data. Additionally, we explored
the information about the site's population, location, and current dominant SARS-CoV-2 variant
as predictors of the relationship between wastewater concentration and case positivity. Our
findings revealed that the linear relationship between the log of sewage concentration of
SARS-CoV-2 and the log of reported cases was significantly improved when controlling for the
site's population and LOD.
These findings contribute to our understanding of the factors that can impact the
accuracy and precision of wastewater-based epidemiology for COVID-19 detection. By
identifying and incorporating appropriate normalizers, we can enhance the accuracy and
precision of wastewater-based epidemiology, thus facilitating early detection and response to
potential outbreaks, and supporting effective public health interventions in the ongoing fight
against the pandemic. Further research in this area is warranted to optimize the use of
normalizers and improve the precision of wastewater-based COVID-19 detection methods.
...
One of the key advantages of wastewater surveillance is its ability to provide population-level data, as wastewater samples can be collected from sewage treatment plants or other collection points that serve large populations. This makes it a cost-effective and non-invasive method for monitoring the overall prevalence of Covid-19 in a community, especially in areas with limited testing resources or where asymptomatic cases may be missed through clinical testing alone. 
...
Graph_DF <- data.select.1%>%
  ...
  mutate(flow_avg_conc = log1p(flow * avg_sars_cov2_conc),
         ...
         case_rate = log1p(case_rate),
         LOD = (!n1_sars_cov2_lod) & (!n2_sars_cov2_lod))
...
df_LOD <- Graph_DF%>%
  ...
  mutate(flow = scale(flow))%>%
  ungroup()%>%
  mutate(n_case_rate = case_rate  - .676*log(pop))%>%
  mutate(low_flow = flow < -0)
...
```{r plot 2}
temp = Graph_DF%>%
  mutate(true_lod = avg_sars_cov2_conc > 6.5,
         LOD = ifelse(true_lod, "Above LOD", "Below LOD"),)%>%
  ggplot(aes(x = exp(avg_sars_cov2_conc), y = exp(case_rate -.676*log(pop))))+
  #geom_point(size = size, alpha = alpha,)+
  stat_bin2d()+
  scale_y_log10()+
  scale_x_log10()+
  #theme_minimal()+
  labs(y = "Normalized Average of Weekly New Cases", x = "SARS-CoV-2 Particles per Liter",
       title = "Relationship Post Population Normalization")
plotly::ggplotly(temp)

Graph_DF%>%
  mutate(true_lod = avg_sars_cov2_conc > 7,
         casezzz = case_rate -.676*log(pop))%>%
  lm(casezzz ~ avg_sars_cov2_conc*true_lod, data = .)%>%
  summary()
```

```{r lm_results}

#covariant_control_lm <- df_LOD%>%
#  mutate(n_case_rate = case_rate  - .676*log(pop))%>%
#  filter(avg_sars_cov2_conc > log(6e4))%>%
#  filter(avg_sars_cov2_conc < log(1e6))%>%
#  lm(n_case_rate ~ PMMoV, data  = .)
#summary(covariant_control_lm)

plot_lm <- df_LOD%>%
  select(wwtp_name, pcr_type, flow, case_rate, pop, avg_sars_cov2_conc)%>%
  #mutate(n_case_rate = case_rate  - .5*log(pop))%>%
  filter(avg_sars_cov2_conc > log(6e4))%>%
  filter(avg_sars_cov2_conc < log(1e6))%>%
  lm(case_rate ~ avg_sars_cov2_conc + log(pop), data  = .)
```

```{r plot 3}
df_LOD%>%
  modelr::add_predictions(plot_lm)%>%
  filter(avg_sars_cov2_conc > log(3e4))%>%
  filter(avg_sars_cov2_conc < log(5e7))%>%
  ggplot()+
  stat_binhex(aes(x = exp(avg_sars_cov2_conc), y = exp(n_case_rate)))+
  #geom_point(size = size, alpha = alpha)+
  #geom_line(aes(y = exp(pred)), color = "white", size = 1.5)+
  scale_y_log10()+
  scale_x_log10()+
  #theme_minimal()+
  labs(y = "Normalized Average of Weekly New Cases", x = "SARS-CoV-2 Particles per Liter",
       title = "Relationship Post Normalization and LOD Filter")
...
```{r}
Graph_DF%>%
  lm( case_rate ~ avg_sars_cov2_conc , data = .)%>%
  summary()
...
simple_lm_model <- lm(n_case_rate ~ avg_sars_cov2_conc, data  = rm_data)
library(randomForest)
rm_data <- rm_data%>%
  modelr::add_predictions(simple_lm_model)%>%
  mutate(resid = abs(n_case_rate - pred),
         PMMoV = log(PMMoV + 2),
         tss = as.numeric(tss),
  ...
  select(wwtp_name, resid, regions, PMMoV, flow, ph, pcr_type, pop_group)%>%
  group_by(wwtp_name, pcr_type)%>%
...
#n1_sars_cov2_lod, n2_sars_cov2_lod, LOD, pcr_type, main_variant
...
gen_R2_params <- function(lm_formula, df){
  ...
  return(c(round(summary(lm_run)$adj.r.squared, 3),
           round(mean(lm_run$residuals^2, na.rm = TRUE),3),
           length(coef(lm_run))))
}
```
...
{
  output_DF <- data.frame("Type" = c("adjusted R^2", "mse", "num_param"),
             `All categorical interaction model` = gen_R2_params(full_formula, Graph_DF),
             "All categorical indirect model" = gen_R2_params(lm_full_inder_formula, Graph_DF),
             "Sub data all interaction model" = gen_R2_params(lm_true_formula, df_LOD),
           "Sub data indirect interaction model"= gen_R2_params(lm_true_inder_formula, df_LOD),
             "Base relationship" = gen_R2_params(simple_formula, Graph_DF),
           check.names = FALSE)
